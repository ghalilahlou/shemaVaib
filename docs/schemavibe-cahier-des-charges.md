...
**Simuler un état passé se fait à la création, jamais par une mise à jour** — dès qu'un test porte sur un seuil ou sur de l'ancienneté, il doit produire des données déjà datées. Reculer un horodatage par un `UPDATE` ne fonctionne pas : les triggers qui entretiennent `maj_le` le remettent à l'instant présent, et la donnée « vieillie » redevient aussitôt récente. Le test constate alors l'état d'aujourd'hui en croyant éprouver celui d'il y a deux semaines, et passe sans rien démontrer. Corollaire : chaque seuil est franchi dans les deux sens — juste en deçà, puis juste au-delà — sans quoi rien ne prouve qu'il se déclenche au bon moment.
...
| SV-001 | Authentification | ✅ Terminé |
...
Ce tableau est une archive depuis le 24 septembre 2026 : le backlog de démarrage a été importé dans la plateforme (ticket SV-012), et les tickets suivants y sont créés et suivis, jamais ici.
...
- **v0.12 (2026-09-23, consignée après coup)** — Bibliothèque de patterns (SV-008) : les patterns de la section 5.2 sont posés par une migration idempotente et non par `seed.sql`, un fichier de seed n'atteignant que l'environnement local alors que la Definition of Ready dépend de ce référentiel partout. Huit patterns et non sept comme l'annonce la ligne SV-008 de la section 22, écrite avant l'ajout de Regression Radius par la veille (v0.6). Les catégories (planification, qualité, exécution, revue, sécurité, orchestration) sont une taxonomie introduite à cette occasion, la section 5.2 n'en définissant aucune.
...
- **v0.20 (2026-09-24)** — Le tableau de suivi de la section 24 devient une archive : la règle 6 est appliquée, le backlog de démarrage est importé dans la plateforme (SV-012) et les tickets suivants n'y sont plus consignés. L'import est rejouable de bout en bout — identifiants fixes pour le projet et les quatorze tickets, écritures en `upsert` — afin qu'il puisse être relancé contre un projet distant sans repartir de zéro et sans détacher les soumissions ou messages déjà rattachés. Il fait autorité sur le contenu du backlog : une divergence introduite à la main est corrigée au prochain import, jamais laissée en place.
